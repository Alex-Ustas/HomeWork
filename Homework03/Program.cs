...
// Есть массив с городами, вводим любую букву и ищем есть ли в каком-то городе
// из массива эта буква
// Ещё доп условие конечно, в каком городе чаще встречается эта буква тот и выводим,
// если одинаково, то вывести несколько
...
cities[3] = "Washington";
cities[4] = "Tokio";
int[] symcity = new int[5];
for (int i = 0; i < 5; i++) symcity[i] = 0;
...
for (int i = 0; i < cities.Count(); i++)
{
    ...
    if (max < symcity[i]) max = symcity[i];
}
for (int i = 0; i < cities.Count(); i++)
{
    if (max == symcity[i])
    {
        Console.WriteLine(cities[i] + " - " + symcity[i]);
    }
}
