...
double[] FillArrayTask1()
{
    ...
    double[] array = new double[random.Next(3, 9)];
...
double[] FillArrayTask2() //Метод создания и наполнения массива для задачи 2
{
    ...
    Random random = new Random();
    double[] array = new double[random.Next(4, 9)];
...
double[] FillArrayTask3() //Метод создания и наполнения массива для задачи 3
{
    ...
    for (int i = 0; i < array.Length; i++)
    {
        array[i] = Math.Round((random.NextDouble() + random.Next(-100, 100)), 2);
    }
